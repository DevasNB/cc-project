<?php
    ...
    if (isset($_GET["id"])) {
        ...
        $sql = "SELECT * FROM carros.rental WHERE id='$id'";
        $result = $conn->query($sql);

        if ($result && $result->num_rows > 0) {
            $rental = $result->fetch_assoc();
        } else {
            echo "Nenhum alguger encontrado com este ID.";
        }
    } else {
        echo "ID do aluguer não fornecido na URL.";
    }
    ...
    <nav class="navbar navbar-expand-lg bg-body-tertiary">
      ...
      <div class="container">
        ...
        <div class="col-12">
            ...
            <form method="POST" action="queries/rental/update_rental.php">
            <?php if(isset($rental)) { ?>
                <div class="form-group">
                    <label for="cars">Modelo de carro</label>
                    <select name="id_car" id="cars" required>
                        <option value="<?= $rental['id_car'] ?>">Selecione um modelo</option>
                ...
                <div class="form-group">
                    ...
                    <input type="number" class="form-control" name="kilo_meters" value="<?= $rental['kilo_meters'] ?>" required>
                </div>
                <div class="form-group mt-2">
                    <label for="date_start">Data de início</label>
                    <input type="date" class="form-control" name="date_start" value="<?= $rental['date_start'] ?>" required>
                </div>
                <div class="form-group mt-2">
                    <label for="date_end">Data de fim</label>
                    <input type="date" class="form-control" name="date_end" value="<?= $rental['date_end'] ?>" required>
                </div>
                <input type="hidden" name="id" value="<?= $rental['id'] ?>">
                <button type="submit" class="btn btn-primary btn-block mt-2">Atualizar</button>
                <?php } else { ?>
                        <p>Nenhum aluguer para editar.</p>
                    <?php } ?>
            </form>
            </div>
        </div>
